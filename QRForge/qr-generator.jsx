...
const CDNJS_QRCODE = "https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js";
...
// Finder = bg rect (gap) + ring (outer-inner evenodd) + inner filled dot
...
function dotPath(col, row, mod, style) {
  ...
  switch(style) {
    ...
    case "square": return `M ${lx} ${ly} h ${ds} v ${ds} h -${ds} Z`;
...
function generateSTL(matrix, opts) {
  ...
  let baseTris="", modTris="", accTris="";

  if (reliefMode === "raised") {
    // Classic: flat base plate + raised dark modules on top
    ...
    // Inset (default): thick slab with LIGHT modules raised, dark modules recessed
    // Material 1 = background slab (full height = baseH + moduleH)
    // Material 2 = light/gap fills raised to match slab top (so dark areas are recessed pockets)
    // Implementation: base is full-height slab; modules (dark) are NOT raised,
    // instead light squares ARE raised as the top surface.
    // Simpler equivalent: full slab at (baseH+moduleH), then dark modules are
    // recessed by NOT adding them. Light cells get a raised box from baseH to baseH+moduleH.
    // For printability: base = full board at baseH; light cells = raised moduleH on top
    baseTris += box(0,0,0,boardSize,boardSize,baseH);
    for(let r=0;r<n;r++) for(let c=0;c<n;c++) {
      if(matrix[r][c]) continue; // skip dark — they stay recessed at baseH
      modTris += box(off+c*modMM, off+r*modMM, baseH, modMM, modMM, moduleH);
    }
  }

  // accessories (go into base mesh for material 1)
  ...
  if(accessory==="strap") {
    ...
    baseTris += box(lx-3, -lugDepth, 0, strapW+6, 2.5, lugH); // bar housing top
    // bottom lugs
  ...
  if(accessory==="stand") {
    ...
    // leg (lays flat for printing, attach at hinge)
...
// ─── Built-in icon presets (inline SVG as data URLs) ─────────────────────────
const ICON_PRESETS = [
...
export default function QRForge() {
  ...
  const [reliefMode,  setReliefMode]  = useState("inset"); // "inset" | "raised"
  ...
  // Keep preset icon logo in sync when iconColor or selection changes
  useEffect(() => {
    if (selectedIcon) {
  ...
  useEffect(() => {
    ...
    const s = document.createElement("script"); s.src=CDNJS_QRCODE; s.onload=()=>setQrLoaded(true); document.head.appendChild(s);
  }, []);

  const buildMatrix = useCallback(() => {
    if (!qrLoaded||!window.QRCode) return;
    try {
      const div=document.createElement("div"); div.style.cssText="visibility:hidden;position:fixed"; document.body.appendChild(div);
      const qr=new window.QRCode(div,{text:buildQRData(ctab,fields),width:1,height:1,correctLevel:window.QRCode.CorrectLevel[ecLevel]});
      const m=qr._oQRCode; if(!m){document.body.removeChild(div);return;}
      const cnt=m.getModuleCount(); const mtx=[];
      for(let r=0;r<cnt;r++){const row=[];for(let c=0;c<cnt;c++)row.push(m.isDark(r,c));mtx.push(row);}
      setQrMatrix(mtx); document.body.removeChild(div);
    } catch(e){}
  }, [qrLoaded,ctab,fields,ecLevel]);
  ...
  const css=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    input[type=text],input[type=url],input[type=password],input[type=email],input[type=tel],textarea,select{
      width:100%;padding:8px 11px;background:var(--s);border:1px solid var(--bd);border-radius:8px;color:var(--t);font-family:'Syne',sans-serif;font-size:.83rem;outline:none;transition:.15s}
      ...
      <div className="shell">
        ...
        <div className="L">
          ...
          {mainTab==="Design" && <>
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                ...
                <TooltipBtn id="ctype" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Content Type" sub=""
                  desc="Selects the data format encoded in the QR. Each type formats the payload differently so phones can take the right action — open a URL, join WiFi, dial a number, save a contact, etc."/>
              </div>
              <select value={ctab} onChange={e=>setCtab(e.target.value)} style={{width:"100%"}}>
            ...
            <div className="sec">
              ...
              {ctab==="appstore"  && <>
                <input type="url" placeholder="App Store or Play Store URL" value={fields.appUrl} onChange={e=>ff("appUrl",e.target.value)}/>
                <div className="notice" style={{marginTop:0}}>Paste the full URL from the App Store, Google Play, or F-Droid listing page.</div>
              </>}
              ...
              {ctab==="bluetooth" && <input type="text" placeholder="BT MAC address (00:11:22:33:44:55)" value={fields.btAddress} onChange={e=>ff("btAddress",e.target.value)}/>}
              {ctab==="phone"     && <input type="tel" placeholder="+1 555 000 0000" value={fields.phone} onChange={e=>ff("phone",e.target.value)}/>}
              {ctab==="sms"       && <>
                <input type="tel" placeholder="Phone number" value={fields.smsPhone} onChange={e=>ff("smsPhone",e.target.value)}/>
                <textarea placeholder="Pre-filled message (optional)" value={fields.smsMsg} onChange={e=>ff("smsMsg",e.target.value)}/>
              </>}
              {ctab==="email"     && <>
                <input type="email" placeholder="To address" value={fields.emailTo} onChange={e=>ff("emailTo",e.target.value)}/>
                <input type="text" placeholder="Subject (optional)" value={fields.emailSubj} onChange={e=>ff("emailSubj",e.target.value)}/>
                <textarea placeholder="Body (optional)" value={fields.emailBody} onChange={e=>ff("emailBody",e.target.value)}/>
              </>}
              ...
              {ctab==="location"  && <>
                <div className="r2">
                  <input type="text" placeholder="Latitude (e.g. 37.7749)" value={fields.geoLat} onChange={e=>ff("geoLat",e.target.value)}/>
                  <input type="text" placeholder="Longitude (e.g. -122.4194)" value={fields.geoLng} onChange={e=>ff("geoLng",e.target.value)}/>
                </div>
                <input type="text" placeholder="Label / place name (optional)" value={fields.geoLabel} onChange={e=>ff("geoLabel",e.target.value)}/>
              </>}
              {ctab==="calendar"  && <>
                <input type="text" placeholder="Event title" value={fields.calTitle} onChange={e=>ff("calTitle",e.target.value)}/>
                <div className="r2">
                  <div><div style={{fontSize:".68rem",color:"var(--mu)",marginBottom:3}}>Start</div><input type="datetime-local" value={fields.calStart} onChange={e=>ff("calStart",e.target.value)}/></div>
                  <div><div style={{fontSize:".68rem",color:"var(--mu)",marginBottom:3}}>End</div><input type="datetime-local" value={fields.calEnd} onChange={e=>ff("calEnd",e.target.value)}/></div>
                </div>
                <input type="text" placeholder="Location (optional)" value={fields.calLoc} onChange={e=>ff("calLoc",e.target.value)}/>
                <textarea placeholder="Description (optional)" value={fields.calDesc} onChange={e=>ff("calDesc",e.target.value)}/>
              </>}
              {ctab==="crypto"    && <>
                <div className="r2">
                  <select value={fields.cryptoCoin} onChange={e=>ff("cryptoCoin",e.target.value)}>
                    <option value="bitcoin">Bitcoin (BTC)</option>
                    <option value="ethereum">Ethereum (ETH)</option>
                    <option value="litecoin">Litecoin (LTC)</option>
                    <option value="monero">Monero (XMR)</option>
                  </select>
                  <input type="text" placeholder="Amount (optional)" value={fields.cryptoAmt} onChange={e=>ff("cryptoAmt",e.target.value)}/>
                </div>
                <input type="text" placeholder="Wallet address" value={fields.cryptoAddr} onChange={e=>ff("cryptoAddr",e.target.value)} style={{fontFamily:"'DM Mono',monospace",fontSize:".75rem"}}/>
              </>}
            ...
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                ...
                <TooltipBtn id="ec" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Error Correction"
                  sub="How much damage the QR can survive"
                  desc="Higher levels let scanners recover the code even if it's partially obscured or damaged — but produce a denser, larger QR. Use H when adding a center logo."/>
              </div>
            ...
            <div className="sec">
              ...
              <div className="rng">
                <label style={{display:"flex",alignItems:"center",gap:0}}>Output size
                  <TooltipBtn id="outsize" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Output Size" sub="pixels"
                    desc="Width and height of the exported image in pixels. Has no effect on scannability — only visual resolution. For print, use SVG export instead."/>
                </label><input type="range" min={128} max={1024} step={8} value={size} onChange={e=>setSize(+e.target.value)}/><span className="rv">{size}px</span></div>
              <div className="rng">
                <label style={{display:"flex",alignItems:"center",gap:0}}>
                  Outer border
                  <TooltipBtn id="quietzone" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Quiet Zone" sub="aka: quiet zone / margin"
                    desc="The blank border surrounding the QR code. Scanners use it to detect where the code begins and ends. The QR spec recommends at least 4 modules minimum."/>
                </label>
                <input type="range" min={0} max={10} step={1} value={margin} onChange={e=>setMargin(+e.target.value)}/>
                <span className="rv">{margin} mod</span>
              </div>
            </div>
            ...
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                ...
                <TooltipBtn id="modshape" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Module Shape" sub="aka dot style"
                  desc="The shape of each dark square in the QR grid. Square is the standard and most scannable. Rounded and circle are widely supported. Diamond and star are decorative — test before printing."/>
              </div>
            ...
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                <span className="lbl">Finder Pattern Corners</span>
                <TooltipBtn id="finder" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Finder Patterns" sub="the three corner squares"
                  desc="The three large squares in the corners of every QR code. Scanners use them to detect position and orientation. Rounded corners are cosmetic only and don't affect scannability."/>
              </div>
              <div className="chips">
            ...
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                ...
                <TooltipBtn id="colors" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Colors" sub=""
                  desc="Background is the light areas; Foreground is the dark QR modules. Finder Accent controls the three corner squares independently. Ensure strong contrast — low contrast codes fail to scan."/>
              </div>
            ...
            <div className="sec">
              ...
              <label className="toggle-row" onClick={()=>{ if(logoEnabled){setLogoEnabled(false);setLogo(null);setSelectedIcon(null);} else setLogoEnabled(true); }}>
                <span style={{display:"flex",alignItems:"center",gap:0,pointerEvents:"none"}}>
                  ...
                  <span style={{pointerEvents:"all"}}><TooltipBtn id="logo" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Center Logo / Icon" sub=""
                    desc="Overlays an image or icon in the middle of the QR. Requires Error Correction H (30%) so the covered modules can still be recovered by scanners."/></span>
                </span>
              ...
              <input ref={logoRef} type="file" accept="image/*" style={{display:"none"}}
                onChange={e=>{
                  ...
                  reader.onload=ev=>{ setLogo(ev.target.result); setSelectedIcon(null); };
              ...
              {logoEnabled && <>
                  ...
                  {ICON_PRESETS.map(ic => {
                    ...
                    return (
                      <div key={ic.id} className={"icon-opt"+(isOn?" on":"")}
                        onClick={()=>{
                          if (isOn) { setSelectedIcon(null); setLogo(null); }
                          else { setSelectedIcon(ic.id); setLogo(svgToDataURL(ic.svg, iconColor)); }
                        }}>
                ...
                {logo && (
                  <div className="cr">
                    <input type="color" value={iconColor} onChange={e=>{ setIconColor(e.target.value); if(selectedIcon){const ic=ICON_PRESETS.find(i=>i.id===selectedIcon);if(ic)setLogo(svgToDataURL(ic.svg,e.target.value));}}}/>
                    <span className="cl">Icon color<span className="ch">{iconColor}</span></span>
                  </div>
                )}
          ...
          {mainTab==="Export" && <>
            ...
            {/* ── Mode switch — always at top ── */}
            <div className="sec">
              <div style={{display:"flex",alignItems:"center",gap:0}}>
                <span className="lbl">Mode</span>
                <TooltipBtn id="expfmt" activeId={ecTooltip} setActive={setEcTooltip}
                  title="Export Mode" sub=""
                  desc="Photo exports a flat image — PNG/JPG for screens, SVG vector for print and laser cutting. 3D Model exports an STL for FDM printing with optional multi-material support."/>
              </div>
              <div className="exp-mode-switch">
                <button className={"exp-mode"+(exportFmt!=="stl"?" on":"")}
                  onClick={()=>{ if(exportFmt==="stl") setExportFmt("png"); }}>
                  <span className="exp-mode-icon">🖼</span>
                  <span className="exp-mode-label">Photo</span>
                </button>
                <button className={"exp-mode"+(exportFmt==="stl"?" on":"")}
                  onClick={()=>setExportFmt("stl")}>
                  <span className="exp-mode-icon">⬡</span>
            ...
            {exportFmt === "stl" && <>
              <div className="sec">
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  <span className="lbl">Model Dimensions</span>
                  <TooltipBtn id="moddim" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Model Dimensions" sub="live estimates"
                    desc="Calculated from your current QR matrix size and geometry settings below. Board size is the full outer footprint; total height is base + module height combined."/>
                </div>
                <div className="stats">
              ...
              <div className="sec">
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  <span className="lbl">Relief Mode</span>
                  <TooltipBtn id="relief" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Relief Mode" sub=""
                    desc="Inset: background is a flat slab and dark modules are recessed pockets — great for two-color prints. Raised: classic emboss where dark modules protrude up from the base."/>
                </div>
                <div className="chips">
                  <button className={"chip"+(reliefMode==="inset"?" on":"")} onClick={()=>setReliefMode("inset")}>
                    Inset <span style={{fontSize:".68rem",opacity:.5}}>— recessed dark</span>
                  </button>
                  <button className={"chip"+(reliefMode==="raised"?" on":"")} onClick={()=>setReliefMode("raised")}>
                    Raised <span style={{fontSize:".68rem",opacity:.5}}>— embossed dark</span>
                  </button>
                </div>
              </div>
              ...
              <div className="sec">
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  <span className="lbl">Geometry</span>
                  <TooltipBtn id="geom" activeId={ecTooltip} setActive={setEcTooltip}
                    title="3D Geometry" sub=""
                    desc="Module size controls the XY footprint of each QR square. Base height is the flat plate underneath. Module height is how far the raised/recessed squares protrude. Margin adds blank border modules around the code."/>
                </div>
              ...
              <div className="sec">
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  <span className="lbl">Multi-Material (MMU / AMS)</span>
                  <TooltipBtn id="mmu" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Multi-Material Export" sub="MMU / AMS / Palette"
                    desc="Exports two named STL solids in one file: the base plate and the QR modules. Import into PrusaSlicer or Bambu Studio, split to parts, and assign a different filament color to each body."/>
                </div>
                ...
                {multiMat && <div className="notice">Works with Prusa MMU3, Bambu AMS, Mosaic Palette, and any multi-extruder setup.</div>}
              </div>

              <div className="sec">
                <div style={{display:"flex",alignItems:"center",gap:0}}>
                  <span className="lbl">Attachment / Accessory</span>
                  <TooltipBtn id="acc" activeId={ecTooltip} setActive={setEcTooltip}
                    title="Attachment / Accessory" sub=""
                    desc="Adds a physical attachment geometry to the STL. Keychain adds a torus ring; Strap Lugs add spring-bar housings for watch straps; Kickstand adds an angled rear leg that prints flat."/>
                </div>
                <div className="acc-grid">
                  {[
                    {id:"none",    icon:"✕", name:"None",      desc:"Clean board only"},
                    {id:"keychain",icon:"🔑", name:"Keychain",  desc:"Torus ring at top for split ring or cord"},
                    {id:"strap",   icon:"⌚", name:"Strap Lugs", desc:"Spring bar lugs for 20mm watch strap"},
                    {id:"stand",   icon:"🖥️", name:"Kickstand",  desc:"Rear angled leg, prints flat"},
                  ].map(a=>(
              ...
              <div className="notice">STL units are millimeters. Recommended slicer settings: 0.2mm layers, 3 perimeters, 20% gyroid infill.</div>
            </>}

            {/* ── Download button — always at bottom ── */}
            <div className="sec">
              <button className="eb" style={{width:"100%"}} onClick={doExport}>
                ↓ Download {exportFmt==="stl"?"STL":exportFmt.toUpperCase()}
              </button>
        ...
        <div className="R">
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <button onClick={()=>setPreviewCollapsed(p=>!p)} style={{background:"none",border:"1px solid var(--bd)",borderRadius:6,color:"var(--mu)",fontSize:".7rem",fontFamily:"inherit",padding:"3px 9px",cursor:"pointer",transition:".15s"}}
